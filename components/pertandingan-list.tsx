...
export function PertandinganList({ initialMatches }: PertandinganListProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section */}
      <section className="bg-gradient-to-r from-orange-600 to-red-600 text-white py-12 md:py-20 mt-16 relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              <div className="flex items-center justify-center gap-2 md:gap-3 mb-4 md:mb-6">
                <div className="p-2 md:p-3 bg-white/10 rounded-2xl backdrop-blur-sm">
                  <Trophy className="w-6 h-6 md:w-10 md:h-10" />
                </div>
                <h1 className="text-2xl md:text-6xl font-bold tracking-tight">Pertandingan Bola Voli</h1>
              </div>
              <p className="text-sm md:text-2xl text-orange-100 leading-relaxed mb-6 md:mb-8 font-light max-w-xs md:max-w-none mx-auto">
                Ikuti semua pertandingan bola voli Sulawesi Utara secara real-time
              </p>
              <div className="flex flex-col md:flex-row items-center justify-center gap-3 md:gap-4 text-orange-100 bg-white/10 backdrop-blur-md py-2 px-4 md:px-6 rounded-xl md:rounded-full inline-flex mx-auto w-full md:w-auto max-w-xs md:max-w-none">
                <div className="flex items-center gap-2">
                  <Clock className="w-3.5 h-3.5 md:w-4 md:h-4" />
                  <span className="text-xs md:text-sm font-medium">Terakhir diperbarui: {formattedLastUpdate}</span>
                </div>
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="text-white hover:bg-white/20 h-auto py-1 px-2 text-xs md:text-sm w-full md:w-auto justify-center"
                >
                  <RefreshCw className={cn("h-3 w-3 md:h-3.5 md:w-3.5 mr-1.5", isLoading && "animate-spin")} />
                  Perbarui
                </Button>
              </div>
            </motion.div>
      ...
      <div className="container mx-auto px-4 py-6 md:py-8 -mt-8 md:-mt-10 relative z-20">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2, duration: 0.6 }}
          className="bg-white rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6 mb-6 md:mb-8"
        >
          <div className="flex flex-col lg:flex-row gap-3 md:gap-4 items-center">
            ...
            <div className="relative flex-1 w-full">
              ...
              <Input
                ...
                className="pl-9 md:pl-10 bg-white/50 border-gray-200 focus:border-orange-500 focus:ring-orange-500 transition-all text-sm h-9 md:h-10"
              />
            </div>
            ...
            <div className="w-full lg:w-64">
              <Select value={selectedLeague} onValueChange={setSelectedLeague}>
                <SelectTrigger className="bg-white/50 border-gray-200 text-gray-900 text-sm h-9 md:h-10">
                  <Filter className="h-3.5 w-3.5 md:h-4 md:w-4 mr-2 text-gray-500" />
          ...
          {/* Active Filters & Tabs */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-3 md:mt-4 pt-3 md:pt-4 border-t border-gray-100 gap-3 md:gap-4">
            {/* Tabs */}
            <div className="flex p-1 bg-gray-100/50 rounded-lg md:rounded-xl w-full sm:w-auto overflow-x-auto no-scrollbar">
              <button
                onClick={() => setActiveTab("live")}
                className={cn(
                  "flex-1 sm:flex-none flex items-center justify-center gap-1.5 md:gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-md md:rounded-lg text-xs md:text-sm font-medium transition-all duration-200 whitespace-nowrap",
                  activeTab === "live"
                    ? "bg-white text-orange-600 shadow-sm"
                    : "text-gray-500 hover:text-gray-700 hover:bg-gray-200/50"
                )}
              >
                <div className={cn("w-1.5 h-1.5 md:w-2 md:h-2 rounded-full", activeTab === "live" ? "bg-orange-600 animate-pulse" : "bg-gray-400")} />
                Langsung
                <span className={cn("ml-1 md:ml-1.5 text-[10px] md:text-xs py-0.5 px-1 md:px-1.5 rounded-md", activeTab === "live" ? "bg-orange-100 text-orange-700" : "bg-gray-200 text-gray-600")}>
                  {liveMatches.length}
                </span>
              </button>
              <button
              ...
              >
                ...
                Akan Datang
                <span className={cn("ml-1 md:ml-1.5 text-[10px] md:text-xs py-0.5 px-1 md:px-1.5 rounded-md", activeTab === "upcoming" ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-600")}>
              ...
              >
                ...
                Selesai
                <span className={cn("ml-1 md:ml-1.5 text-[10px] md:text-xs py-0.5 px-1 md:px-1.5 rounded-md", activeTab === "finished" ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-600")}>
